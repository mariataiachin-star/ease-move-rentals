  ...
  <section id="packages" className="py-24 bg-gradient-cream">
    <div className="container mx-auto">
      <div className="mx-auto max-w-2xl text-center">
        <p className="text-sm font-bold uppercase tracking-widest text-primary">Our packages</p>
        <h2 className="mt-3 font-display text-4xl font-bold text-foreground md:text-5xl">
          Three kits. One easy decision.
        </h2>
        <p className="mt-4 text-lg text-muted-foreground">
          All prices below are per month — pick longer rentals to save more.
        </p>
      </div>
      ...
      <div className="mt-16 grid gap-8 lg:grid-cols-3">
        {PACKAGES.map((pkg) => {
          ...
          return (
            <article
              key={pkg.id}
              className={`relative flex flex-col rounded-3xl border bg-card p-8 transition-smooth hover:-translate-y-2 ${
                featured
                  ? "border-primary/40 shadow-warm lg:scale-105"
            ...
            >
              ...
              <h3 className="mt-5 font-display text-2xl font-bold text-foreground">{pkg.name}</h3>
              ...
              <div className="mt-6 flex items-baseline gap-1">
                <span className="font-display text-5xl font-bold text-foreground">€{pkg.monthlyPrice}</span>
